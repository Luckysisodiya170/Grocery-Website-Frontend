...
const OrderHistory = () => {
  ...
  const filteredOrders = orders.filter((order) => {
    const isHistory = ["Delivered", "Cancelled", "Returned", "Partially Delivered"].includes(order.order_status);
  ...
  const renderOrderBox = (order, index, isLast) => {
    ...
    const mainOrderStatus = order.order_status?.toLowerCase();
    const canCancelOrder = ["pending", "confirmed", "placed", "shipped"].includes(mainOrderStatus);

    return (
      <div
        key={order.id}
        ref={isLast ? lastOrderElementRef : null}
        className="bg-white rounded-[24px] mb-8 border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-all"
      >
        {/* Order Header */}
        ...
        <div className="p-6 flex flex-col gap-8">
          {order.items?.map((item) => {
            ...
            // Return logic: Delivered status + is_return_allowed true
            const showItemReturn = itemStatus === "delivered" && item.is_return_allowed;

            return (
              <div key={item.item_id} className="flex flex-col gap-4 pb-6 border-b border-slate-50 last:border-0 last:pb-0">
                <div 
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col md:flex-row justify-between md:items-center gap-4">
                    <div className="flex flex-col">
                      <h4 className="text-[15px] font-black text-slate-800 group-hover:text-cyan-600 transition-colors">{item.product_name}</h4>
                      <p className="text-[11px] font-bold text-slate-400 uppercase">{item.vendor_name}</p>
                      
                      {/* Individual Item Status */}
                      <div className="mt-2 flex items-center gap-3">
                         <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${getStatusStyle(item.status)}`}>
                            {getDisplayStatus(item.status)}
                         </span>
                         <span className="text-[13px] font-black text-slate-700">Qty: {item.quantity}</span>
                      </div>
                    </div>
                ...
                {showItemReturn && (
                  <div className="flex justify-end">
                    <button 
                      ...
                      className="px-6 py-2 bg-slate-900 text-white font-black text-[11px] uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-all shadow-md"
                    >
                      Return This Item ({item.return_days} Days Left)
                    </button>
                  </div>
                )}
              </div>
        ...
        <div className="bg-slate-50/30 px-6 py-5 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[22px] font-black text-cyan-700">₹{Number(order.total_amount).toLocaleString('en-IN')}</span>
          </div>
